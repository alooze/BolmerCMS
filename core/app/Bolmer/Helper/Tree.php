...
class Tree{
	...
	*/
    public static function build($data, $idName, $pidName, $level=0, $levelName = 'levelNode', $childName = 'childNodes'){
        ...
        foreach ($data as $i=>$r) {
            ...
            if($level==0 || $row[$levelName]<=$level){
                ...
                $children[self::getParentLevel($data,$pidName,$level,$pid, $levelName)][$id] = &$row;
            }
    ...
    private static function getParentLevel($data,$pidName,$level,$i=0, $levelName = 'levelNode'){
        if($data[$i][$levelName]>$level){
            $out=self::getParentLevel($data,$pidName,$level,$data[$i][$pidName]);
        }else{
            $out=$data[$i][$pidName];
        }
        return $out;
