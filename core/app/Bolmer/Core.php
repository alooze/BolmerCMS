...
class Core
{
    ...
    {
        $out = array();
        if (empty($this->_version) || !is_array($this->_version)) {
            //include for compatibility modx version < 1.0.10
            include_once BOLMER_MANAGER_PATH . "includes/version.inc.php";
            $this->_version = array();
            $this->_version['version'] = CMS_RELEASE_VERSION;
            $this->_version['branch'] = CMS_NAME;
            $this->_version['release_date'] = CMS_RELEASE_DATE;
            $this->_version['full_appname'] = CMS_FULL_APPNAME;
            $this->_version['new_version'] = $this->getConfig('newversiontext');
        }
        return (!is_null($data) && is_array($this->_version) && isset($this->_version[$data])) ? $this->_version[$data] : $this->_version;
    }
