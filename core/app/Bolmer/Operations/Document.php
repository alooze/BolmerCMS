...
class Document{
    ...
    public function getChildIds($id, $depth= 10, $children= array ()) {
        ...
        if (!count($documentMap_cache)) {
            foreach ($this->_modx->documentMap as $document) {
        ...
        if (isset($documentMap_cache[$id])) {
            ...
            foreach ($documentMap_cache[$id] as $childId) {
                $pkey = (strlen($this->_modx->aliasListing[$childId]['path']) ? "{$this->_modx->aliasListing[$childId]['path']}/" : '') . $this->_modx->aliasListing[$childId]['alias'];
    ...
    public function getParentIds($id, $height= 10) {
        ...
        while ( $id && $height-- ) {
            $thisid = $id;
            $id = $this->_modx->aliasListing[$id]['parent'];
            if (!$id) break;
            $parents[$thisid] = $id;
        }
        return $parents;
    ...
    public function getAllChildren($id= 0, $sort= 'menuindex', $dir= 'ASC', $fields= 'id, pagetitle, description, parent, alias, menutitle') {
        $tblsc= $this->_modx->getFullTableName("site_content");
        $tbldg= $this->_modx->getFullTableName("document_groups");
        // modify field names to use sc. table reference
        $fields= 'sc.' . implode(',sc.', preg_replace("/^\s/i", "", explode(',', $fields)));
        $sort= 'sc.' . implode(',sc.', preg_replace("/^\s/i", "", explode(',', $sort)));
        // get document groups for current user
        if ($docgrp= $this->_modx->getUserDocGroups())
            $docgrp= implode(",", $docgrp);
        // build query
        $access= ($this->_modx->isFrontend() ? "sc.privateweb=0" : "1='" . $_SESSION['mgrRole'] . "' OR sc.privatemgr=0") .
        ...
        $resourceArray= array ();
        for ($i= 0; $i < @ $this->_modx->db->getRecordCount($result); $i++) {
            array_push($resourceArray, @ $this->_modx->db->getRow($result));
        }
    ...
    public function getPageInfo($pageid= -1, $active= 1, $fields= 'id, pagetitle, description, alias') {
        if ($pageid == 0) {
            ...
            $sql= "SELECT $fields
                    ...
                    LIMIT 1 ";
            $result= $this->_modx->db->query($sql);
            $pageInfo= @ $this->_modx->db->getRow($result);
            return $pageInfo;
        }
    }
    ...
    public function getParent($pid= -1, $active= 1, $fields= 'id, pagetitle, description, alias, parent') {
        if ($pid == -1) {
            $pid= $this->_modx->documentObject['parent'];
    ...
    public function getDocumentObject($method, $identifier, $isPrepareResponse=false) {
        ...
        if($method == 'alias') {
            $identifier = $this->_modx->cleanDocumentIdentifier($identifier);
            $method = $this->_modx->documentMethod;
        }
        if($method == 'alias' && $this->_modx->getConfig('use_alias_path') && array_key_exists($identifier, $this->_modx->documentListing)) {
            $method = 'id';
            $identifier = $this->_modx->documentListing[$identifier];
        }
        // get document groups for current user
        if ($docgrp= $this->_modx->getUserDocGroups())
            $docgrp= implode(",", $docgrp);
        // get document
        $access=  "1='" . $_SESSION['mgrRole'] . "'" . ($this->_modx->isFrontend() ? " OR sc.privateweb=0" : " OR sc.privatemgr=0") .
            (!$docgrp ? "" : " OR dg.document_group IN ($docgrp)");
        $sql= "SELECT sc.*
              FROM $tblsc sc
              LEFT JOIN $tbldg dg ON dg.document = sc.id
        ...
        $documentObject= $this->_modx->db->getRow($result);
        if($isPrepareResponse==='prepareResponse') $this->_modx->documentObject = & $documentObject;
        $this->_modx->invokeEvent('OnLoadDocumentObject');
        if ($documentObject['template']) {
            // load TVs and merge with document - Orig by Apodigm - Docvars
            $sql= "SELECT tv.*, IF(tvc.value!='',tvc.value,tv.default_text) as value ";
            $sql .= "FROM " . $this->_modx->getFullTableName("site_tmplvars") . " tv ";
            $sql .= "INNER JOIN " . $this->_modx->getFullTableName("site_tmplvar_templates")." tvtpl ON tvtpl.tmplvarid = tv.id ";
            $sql .= "LEFT JOIN " . $this->_modx->getFullTableName("site_tmplvar_contentvalues")." tvc ON tvc.tmplvarid=tv.id ";
            $sql .= "WHERE tvc.contentid = '" . $documentObject['id'] . "' AND tvtpl.templateid = '" . $documentObject['template'] . "'";
            $rs= $this->_modx->db->query($sql);
            $rowCount= $this->_modx->db->getRecordCount($rs);
            if ($rowCount > 0) {
    ...
    function getTemplateVars($idnames= array (), $fields= "*", $docid= "", $published= 1, $sort= "rank", $dir= "ASC") {
        if (($idnames != '*' && !is_array($idnames)) || count($idnames) == 0) {
            ...
            // get document record
            if ($docid == "") {
                $docid= $this->_modx->documentIdentifier;
                $docRow= $this->_modx->documentObject;
            } else {
            ...
            $rs= $this->_modx->db->query($sql);
            for ($i= 0; $i < @ $this->_modx->db->getRecordCount($rs); $i++) {
                array_push($result, @ $this->_modx->db->getRow($rs));
            }
    ...
    function getDocumentChildrenTVars($parentid= 0, $tvidnames= array (), $published= 1, $docsort= "menuindex", $docsortdir= "ASC", $tvfields= "*", $tvsort= "rank", $tvsortdir= "ASC") {
        ...
        if (!$docs)
            ...
            for ($i= 0; $i < $docCount; $i++) {
                ...
                $rs= $this->_modx->db->query($sql);
                $limit= @ $this->_modx->db->getRecordCount($rs);
                for ($x= 0; $x < $limit; $x++) {
                    array_push($tvs, @ $this->_modx->db->getRow($rs));
                }
    ...
    public function stripAlias($alias) {
        ...
        $results = $this->_modx->invokeEvent('OnStripAlias', array ('alias'=>$alias));
        if (!empty($results)) {
    ...
    public function getIdFromAlias($alias)
    {
        ...
        $tbl_site_content = $this->_modx->getFullTableName('site_content');
        if($this->_modx->getConfig('use_alias_path')==1)
        {
            ...
            {
                if($id===false) break;
                $alias = $this->_modx->db->escape($alias);
                $rs  = $this->_modx->db->select('id', $tbl_site_content, "deleted=0 and parent='{$id}' and alias='{$alias}'");
                if($this->_modx->db->getRecordCount($rs)==0) $rs  = $this->_modx->db->select('id', $tbl_site_content, "deleted=0 and parent='{$id}' and id='{$alias}'");
                $row = $this->_modx->db->getRow($rs);
        ...
        {
            $rs = $this->_modx->db->select('id', $tbl_site_content, "deleted=0 and alias='{$alias}'", 'parent, menuindex');
            $row = $this->_modx->db->getRow($rs);
